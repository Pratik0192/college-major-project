...
const Wishlist = () => {
  ...
  useEffect(() => {
    if (token) {
      setLoading(true);
      getWishlistedItems(token).finally(() => setLoading(false));
    }
  }, [token]);
  ...
  const wishlistedProducts = products.filter(
    (product) => wishlistItems[product._id]
  );
      ...
      {token ? (
        <div>
          {wishlistedProducts.length > 0 ? (
            <div>
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="text-center py-12">
              ...
              <Link to="/" className="btn btn-primary px-6 py-2 rounded-full">
                Continue Shopping
              </Link>
            </div>
          )}
        ...
        <div className="text-center py-12">
          ...
          <Link to="/login" className="btn btn-primary px-6 py-2 rounded-full">
            Login
          </Link>
        </div>
      )}
